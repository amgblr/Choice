...
function ZoomWrapper({ render, slide, offset, rect }) {
  ...
  if (!rendered && isImageSlide(slide)) {
    ...
    rendered = isResponsiveImageSlide(slide) ? React.createElement(ResponsiveImage, { key: slide.src, ...slideProps, slide, rect: offset === 0 ? { width: rect.width * zoom, height: rect.height * zoom } : rect }) : React.createElement(ImageSlide, { key: slide.src, onLoad: (img) => setImageDimensions({ width: img.naturalWidth, height: img.naturalHeight }), ...slideProps });
  }
  if (!rendered)
    return null;
  return React.createElement("div", { ref: zoomWrapperRef, className: clsx(cssClass(CLASS_FULLSIZE), cssClass(CLASS_FLEX_CENTER)), style: offset === 0 ? { transform: `scale(${zoom}) translateX(${offsetX}px) translateY(${offsetY}px)` } : void 0 }, rendered);
}
