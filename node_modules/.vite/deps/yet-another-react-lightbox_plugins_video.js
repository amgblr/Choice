...
function VideoSlide({ slide, offset }) {
  ...
  }, className: clsx(cssClass("video_container"), cssClass(CLASS_FLEX_CENTER)) }, containerRect && React.createElement("video", { ref: setVideoRef, poster, ...scaleWidthAndHeight(), ...resolveBoolean("controls"), ...resolveBoolean("playsInline"), ...resolveBoolean("loop"), ...resolveBoolean("muted"), ...resolveBoolean("playsInline"), ...resolveBoolean("disablePictureInPicture"), ...resolveBoolean("disableRemotePlayback"), ...resolveString("controlsList"), ...resolveString("crossOrigin"), ...resolveString("preload"), onPlay: () => {
    publish(ACTIVE_SLIDE_PLAYING);
  }, onEnded: () => {
    publish(ACTIVE_SLIDE_COMPLETE);
  } }, sources.map(({ src, type }, index) => React.createElement("source", { key: index, src, type })))));
}
    ...
    render: {
      slide: ({ slide, offset, rect }) => {
        ...
        if (isVideoSlide(slide)) {
          return React.createElement(VideoSlide, { key: `${(_a = slide.sources) === null || _a === void 0 ? void 0 : _a.map((source) => source.src).join(" ")}`, slide, offset });
        }
